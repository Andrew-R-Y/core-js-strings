...
function formatTime(minutes, seconds) {
  const fullMinutes = minutes.toString();
  const fullSeconds = seconds.toString();
  return `${fullMinutes.padStart(2, '0')}:${fullSeconds.padStart(2, '0')}`;
}
...
function isPalindrome(str) {
  ...
  for (let i = 0; i < strLowerCase.length; i += 1) {
    const symbol = strLowerCase.charAt(i);
    if (
      symbol !== ' ' &&
      symbol !== ',' &&
      symbol !== '!' &&
  ...
  if (strNoSpaces.length === 1) {
    ...
    const middle = Math.floor(strNoSpaces.length / 2);
    result = true;
    for (let i = 0; i < middle; i += 1) {
      if (
        strNoSpaces.charAt(i) !== strNoSpaces.charAt(strNoSpaces.length - 1 - i)
      ) {
        result = false;
      }
    }
...
function encodeToRot13(str) {
  ...
  const targetAlphabet = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
  let result = '';
  for (let i = 0; i < str.length; i += 1) {
    if (
      str[i] === ' ' ||
      ...
      for (let j = 0; j < 52; j += 1) {
        if (str[i] === baseAlphabet[j]) {
          result += targetAlphabet[j];
        }
      }
    }
  }
